...
- **Eine bewusst strengere Regel als ursprünglich geplant:** `declined` entsteht ausschließlich aus einem vorhandenen Ergebniscode des Terminals ungleich `'0'`. Ein quittierter `HpsClient.abort()` allein genügt dafür nicht mehr — er löst nur eine sofortige, erneute Statusabfrage aus (ohne Backoff), entscheidet aber nichts. Ursprünglich sollte ein quittierter Abbruch für sich schon als `declined` zählen; das wurde zurückgenommen, weil sein Beweiswert allein auf der ungeprüften Annahme ruht, dass `abort()` zuverlässig scheitert, sobald die Karte aufliegt — ein Beweis auf ungeprüfter Annahme ist genau die Struktur des Vorfalls vom 24.08., nur mit vertauschten Vorzeichen. Der Preis: ein sauber vor dem Kartenauflegen abgebrochener Vorgang, auf den das Terminal danach nie mehr einen Ergebniscode nennt, endet nach Ablauf des Budgets als `unresolved` statt als `declined` — bis zu 90 s Wartezeit, Klärung über die Kennung statt sofortiger Wiederholung. Das ist als Reibung gegen eine Doppelbelastung hingenommen, nicht als enger Abwägungsfall; Stellhebel ist das Budget, nicht die Rückkehr zur alten Regel.
- **Cloud-Weg neu im Paket:** `KasseneckApi.hobexGetStatus` fragt den Stand einer Hobex-Cloud-Transaktion ab und ist die Klärstufe hinter `HobexCloudPayments` — sie unterscheidet „Dienst hat geantwortet und kennt die Kennung nicht" (`null`, weiter pollen) von „konnte gar nicht erst fragen" (Transportfehler, wirft). Scheitern drei Statusabfragen in Folge am Transport, endet die Klärung vorzeitig mit `unresolved` statt den Mitarbeiter das volle Budget warten zu lassen. Dazu `KasseneckApi.cardTimeout` (3 Minuten, für `hobexPay`/`hobexRefund`/`hobexGetStatus`) getrennt von `readTimeout` (30 s) — die bisher pauschale 30-Sekunden-Frist war für einen Belege-Cache gedacht und meldete am Terminal eine durchgelaufene Zahlung als Fehlschlag.
- **Beobachter für den Zahlweg:** ein injizierbarer `HpsObserver`-Rückruf meldet Requests, Fehlschläge und den Stand der Klärung. Ohne Protokoll gab es zum Vorfall vom 24.08. keine Daten außer den hobex-Belegen; das soll sich beim nächsten Vorfall nicht wiederholen.
- **Breaking: `HpsClient.cancel()` verlangt jetzt `amount`.** Das Terminal weist einen Void ohne Betrag mit `400 Missing amount` ab.
- **Breaking: `HpsOutcome` heißt jetzt `CardPaymentOutcome`** und liegt in `lib/src/payments/card_payment_outcome.dart` statt im HPS-Baum, weil derselbe dreiwertige Ausgang jetzt auch vom Cloud-Weg geteilt wird. Die Werte `approved`/`declined`/`unresolved` sind unverändert; betroffen sind `HpsResult.outcome`, der Konstruktor von `HpsResult` und der Export aus `hobex_hps.dart`.
...
**Offen, unbestätigt:**
- Die Erreichbarkeit von `hobexGetStatus` unter `api.kasseneck.at/v1` ist **unbestätigt** — die Methode ist bisher nur gegen Mocks geprüft, nicht gegen das Backend.
- Sechs Annahmen über das Verhalten des Terminals sind mangels Testterminal ungeprüft und begleiten diesen Release: ob eine Wiederholung unter derselben Kennung als derselbe Vorgang erkannt wird; ob das Terminal eine Kennung nach einem Verbindungsabbruch überhaupt noch kennt; ob `abort()` zuverlässig scheitert, sobald die Karte aufliegt, und den laufenden Vorgang nicht stört; was `transactionStatus()` auf eine unbekannte Kennung antwortet; was ein leerer `200`-Rumpf tatsächlich bedeutet; ob das Terminal eine nicht rein numerische Kennung annimmt. Bis sie am echten Terminal geprüft sind, gelten die im Code hinterlegten Annahmen.
